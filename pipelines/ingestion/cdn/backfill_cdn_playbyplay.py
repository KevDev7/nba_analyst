"""Backfill CDN play-by-play JSON payloads with targeted current-season controls."""
...
DEFAULT_MAX_GAMES_PER_RUN = 600

SLEEP_SECONDS = 1.9
SLEEP_JITTER_SECONDS = 1.2

COOLDOWN_EVERY_GAMES = 250
COOLDOWN_SECONDS = 180
COOLDOWN_JITTER_SECONDS = 120
...
DESTINATION_PREFIX = "raw/cdn/playbyplay"
# NBA CDN play-by-play is not reliably available for these archived seasons.
...
def destination_key_for_game(game_id: str) -> str:
    """S3 destination key for one game's play-by-play JSON."""
...
def fetch_playbyplay_for_game(game_id: str, session: requests.Session) -> bytes:
    ...
    url = CDN_PLAYBYPLAY_URL_TEMPLATE.format(game_id=cdn_game_id)
...
def main() -> None:
    """Backfill CDN play-by-play JSON from newest selected games to oldest."""
    args = parse_args()
    ...
    print(
        f"Starting CDN PlayByPlay backfill. "
        f"manifest_source={args.manifest_source}, dry_run={args.dry_run}, "
        f"unsupported_historical_season_start_years={sorted(UNSUPPORTED_SEASON_START_YEARS)}"
    )
    ...
    try:
        manifest_games = discover_manifest_games(
            ...
            unsupported_historical_season_start_years=UNSUPPORTED_SEASON_START_YEARS,
        )
        ...
        for index, game in enumerate(selected_games, start=1):
            ...
            try:
                payload = fetch_playbyplay_with_retry(game_id, session)
            except Exception as exc:
                failed += 1
