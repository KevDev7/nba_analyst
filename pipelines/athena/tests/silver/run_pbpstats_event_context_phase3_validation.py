#!/usr/bin/env python3
"""Validate Athena silver pbpstats_event_context_v1 against direct reprojection."""
...
def read_table_rows(s3_client, key: str) -> list[dict[str, Any]]:
...
def compare_game(s3_client, game_id: str, max_examples: int) -> dict[str, Any]:
    ...
    home_team_id, away_team_id = event_context.boxscore_team_orientation_for_game(s3_client, game_id)
    try:
        _, expected_rows, starter_warnings = event_context.project_payload_to_rows(
            payload,
    ...
    except (event_context.ProjectionFailure, InvalidNumberOfStartersException, RuntimeError, ValueError, AttributeError) as exc:
        try:
            actual_rows = read_table_rows(s3_client, f"silver/pbpstats_event_context_v1/game_id={game_id}.parquet")
    ...
    try:
        ...
        try:
            projection_rows = read_table_rows(s3_client, f"silver/pbpstats_event_projection_v1/game_id={game_id}.parquet")
        except ClientError:
            return {
                "game_id": game_id,
                "status": "projection_base_missing",
                "actual_row_count": 0,
        ...
        return {
            "game_id": game_id,
            "status": "missing_actual_table",
            "actual_row_count": 0,
            "expected_row_count": len(expected_rows),
            "mismatch_count": 1,
            "mismatch_examples": [{"field": "__table_presence__", "actual": None, "expected": "present"}],
            "starter_warning_count": len(starter_warnings),
            "orphan_row_count": 0,
            "non_five_player_row_count": 0,
        }
    ...
    projection_key_set = {
        (row.get("game_id"), row.get("event_num"))
    ...
    for event_num in sorted(set(actual_index) | set(expected_index)):
        ...
        if actual_row is None or expected_row is None:
            ...
            continue
        for column in event_context.PROJECTION_COLUMNS:
    ...
    return {
        ...
        "starter_warning_count": len(starter_warnings),
        "orphan_row_count": len(actual_key_set - projection_key_set),
        "non_five_player_row_count": sum(
            1
            for row in actual_rows
            if event_context.list_size(row.get("home_current_player_ids")) != 5
            or event_context.list_size(row.get("away_current_player_ids")) != 5
        ),
    }
...
def main() -> None:
    ...
    for result in sample_results:
        if result.get("status") != "compared":
            continue
        actual_rows = read_table_rows(s3_client, f"silver/pbpstats_event_context_v1/game_id={result['game_id']}.parquet")
        ...
        non_five_player_row_count += int(result["non_five_player_row_count"])

    failures: list[str] = []
    if duplicate_key_count > 0:
        failures.append(f"duplicate_key_count={duplicate_key_count}")
    if orphan_row_count > 0:
        failures.append(f"orphan_row_count={orphan_row_count}")
    mismatch_games = [
        result["game_id"]
        for result in sample_results
        if result.get("status") in {"compared", "missing_actual_table"} and result["mismatch_count"] > 0
    ]
    if mismatch_games:
        failures.append(f"sample_parity_mismatch_games={','.join(mismatch_games)}")

    summary = {
        "table_name": event_context.TABLE_NAME,
