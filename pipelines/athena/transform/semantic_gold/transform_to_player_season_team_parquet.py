...
from .transform_to_player_season_parquet import (
    ...
    _counted_usage_percentage,
    _proxy_usage_percentage,
...
def build_player_season_team_rows_from_player_game_rows(
    ...
    def _rounded_ratio(numerator: object, denominator: object) -> float | None:
    ...
    for row in player_game_rows:
        ...
        current["games_started"] = (to_int_or_none(current.get("games_started")) or 0) + (
            1 if bool(row.get("is_starter")) else 0
        )
        current["minutes_total"] = (to_float_or_none(current.get("minutes_total")) or 0.0) + (
            to_float_or_none(row.get("minutes_played")) or 0.0
        )
        current["points_total"] = to_int_or_none(current.get("points_total")) or 0
        current["points_total"] += to_int_or_none(row.get("points")) or 0
        for total_field, row_field in (
            ...
            ("steals_total", "steals"),
            ("blocks_total", "blocks"),
            ("opponent_blocks_total", "opponent_blocks"),
            ...
            ("fast_break_points_total", "fast_break_points"),
            ("points_in_paint_total", "points_in_paint"),
            ("second_chance_points_total", "second_chance_points"),
            ...
            current[total_field] = (to_int_or_none(current.get(total_field)) or 0) + (
                to_int_or_none(row.get(row_field)) or 0
            )
    ...
    for key in sorted(grouped):
        ...
        minutes_total = round(float(aggregate.get("minutes_total") or 0.0), 1)
        ...
        if usage_percentage is None:
            usage_percentage = _proxy_usage_percentage(
                ...
                player_minutes_total=minutes_total,
