"""
Build gold team_season_percentiles from canonical team season aggregates.

Reads:
  s3://nba-analytics-lakehouse-dev/gold/agg_team_season/agg_team_season.parquet
  s3://nba-analytics-lakehouse-dev/gold/dim_team/dim_team.parquet
  s3://nba-analytics-lakehouse-dev/gold/fct_team_game/fct_team_game.parquet
  s3://nba-analytics-lakehouse-dev/silver/boxscore_team_game.parquet

Writes (full overwrite):
  s3://nba-analytics-lakehouse-dev/gold/team_season_percentiles/team_season_percentiles.parquet
"""

from __future__ import annotations

from datetime import datetime, timezone
from typing import Any

import boto3
import duckdb
import pyarrow as pa
from dotenv import load_dotenv

try:
    from pipelines.athena.transform.gold import deploy_team_season_boxscore_advanced_view as team_advanced_view
    from pipelines.athena.transform.gold.gold_transform_helpers import (
        S3_BUCKET,
        read_parquet_table_from_s3,
        to_int_or_none,
        to_str_or_none,
        write_parquet_to_s3,
    )
    from pipelines.athena.transform.gold.season_percentile_helpers import (
        PercentileMetricSpec,
        attach_percentiles,
        register_arrow_table,
    )
    from pipelines.athena.transform.gold.transform_to_agg_team_season_parquet import TARGET_SCHEMA as TEAM_AGG_SCHEMA
    from pipelines.athena.transform.gold.transform_to_fct_team_game_parquet import TARGET_SCHEMA as TEAM_FACT_SCHEMA
except ImportError:
    import deploy_team_season_boxscore_advanced_view as team_advanced_view  # type: ignore[no-redef]
    from gold_transform_helpers import (  # type: ignore[no-redef]
        S3_BUCKET,
        read_parquet_table_from_s3,
        to_int_or_none,
        to_str_or_none,
        write_parquet_to_s3,
    )
    from season_percentile_helpers import PercentileMetricSpec, attach_percentiles, register_arrow_table  # type: ignore[no-redef]
    from transform_to_agg_team_season_parquet import TARGET_SCHEMA as TEAM_AGG_SCHEMA  # type: ignore[no-redef]
    from transform_to_fct_team_game_parquet import TARGET_SCHEMA as TEAM_FACT_SCHEMA  # type: ignore[no-redef]

load_dotenv(override=True)

TEAM_AGG_SOURCE_KEY = "gold/agg_team_season/agg_team_season.parquet"
DIM_TEAM_SOURCE_KEY = "gold/dim_team/dim_team.parquet"
TEAM_FACT_SOURCE_KEY = "gold/fct_team_game/fct_team_game.parquet"
SILVER_TEAM_SOURCE_KEY = "silver/boxscore_team_game.parquet"
DESTINATION_KEY = "gold/team_season_percentiles/team_season_percentiles.parquet"

RECORD_SOURCE = "gold.agg_team_season|gold.vw_team_season_boxscore_advanced"

DIM_TEAM_REQUIRED_COLUMNS = [
    "team_id",
    "team_abbreviation",
    "team_name",
    "is_current",
]

SILVER_TEAM_REQUIRED_COLUMNS = [
    "gameid",
    "teamid",
    "score",
    "fieldgoalsmade",
    "fieldgoalsattempted",
    "freethrowsmade",
    "freethrowsattempted",
    "threepointersmade",
    "assists",
    "blocks",
    "foulspersonal",
    "steals",
    "turnoverstotal",
    "reboundsoffensive",
    "reboundsteamoffensive",
    "reboundsdefensive",
    "reboundsteamdefensive",
    "reboundstotal",
]

TEAM_PERCENTILE_SPECS = [
    PercentileMetricSpec(
        source_field="win_percentage",
        output_field="win_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="points_for_per_game",
        output_field="points_for_per_game_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="points_against_per_game",
        output_field="points_against_per_game_percentile",
        direction="lower",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="point_differential_per_game",
        output_field="point_differential_per_game_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="field_goals_percentage",
        output_field="field_goals_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="free_throws_percentage",
        output_field="free_throws_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="three_pointers_percentage",
        output_field="three_pointers_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="two_pointers_percentage",
        output_field="two_pointers_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="offensive_rating",
        output_field="offensive_rating_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="defensive_rating",
        output_field="defensive_rating_percentile",
        direction="lower",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="net_rating",
        output_field="net_rating_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="assist_percentage",
        output_field="assist_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="ast_to_turnover_ratio",
        output_field="ast_to_turnover_ratio_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="assist_ratio",
        output_field="assist_ratio_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="offensive_rebound_percentage",
        output_field="offensive_rebound_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="defensive_rebound_percentage",
        output_field="defensive_rebound_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="rebound_percentage",
        output_field="rebound_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="steal_percentage",
        output_field="steal_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="block_percentage",
        output_field="block_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="turnover_ratio",
        output_field="turnover_ratio_percentile",
        direction="lower",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="effective_field_goal_percentage",
        output_field="effective_field_goal_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="three_point_attempt_rate",
        output_field="three_point_attempt_rate_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="free_throw_attempt_rate",
        output_field="free_throw_attempt_rate_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="true_shooting_percentage",
        output_field="true_shooting_percentage_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="pace",
        output_field="pace_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
    PercentileMetricSpec(
        source_field="pie",
        output_field="pie_percentile",
        direction="higher",
        qualifies=lambda row: _team_qualifies(row),
    ),
]

TEAM_PERCENTILE_FIELDS = [spec.output_field for spec in TEAM_PERCENTILE_SPECS]

TARGET_SCHEMA = pa.schema(
    [
        pa.field("team_season_percentiles_sk", pa.int64()),
        pa.field("team_id", pa.int64()),
        pa.field("current_team_sk", pa.int64()),
        pa.field("season_year", pa.string()),
        pa.field("season_start_year", pa.int64()),
        pa.field("raw_season_type_code", pa.string()),
        pa.field("season_type", pa.string()),
        *[pa.field(field_name, pa.int64()) for field_name in TEAM_PERCENTILE_FIELDS],
        pa.field("record_source", pa.string()),
        pa.field("created_at_utc", pa.timestamp("us", tz="UTC")),
        pa.field("updated_at_utc", pa.timestamp("us", tz="UTC")),
    ]
)


def _team_qualifies(row: dict[str, Any]) -> bool:
    return (to_int_or_none(row.get("games_played")) or 0) > 0


def _build_team_advanced_source_table(
    agg_team_table: pa.Table,
    dim_team_table: pa.Table,
    team_fact_table: pa.Table,
    silver_team_table: pa.Table,
) -> pa.Table:
    team_metric_selects = [
        'ats."win_percentage"',
        'ats."points_for_per_game"',
        'ats."points_against_per_game"',
        'ats."point_differential_per_game"',
        'ats."field_goals_percentage"',
        'ats."free_throws_percentage"',
        'ats."three_pointers_percentage"',
        'ats."two_pointers_percentage"',
        'adv."offensive_rating"',
        'adv."defensive_rating"',
        'adv."net_rating"',
        'adv."assist_percentage"',
        'adv."ast_to_turnover_ratio"',
        'adv."assist_ratio"',
        'adv."offensive_rebound_percentage"',
        'adv."defensive_rebound_percentage"',
        'adv."rebound_percentage"',
        'adv."steal_percentage"',
        'adv."block_percentage"',
        'adv."turnover_ratio"',
        'adv."effective_field_goal_percentage"',
        'adv."three_point_attempt_rate"',
        'adv."free_throw_attempt_rate"',
        'adv."true_shooting_percentage"',
        'adv."pace"',
        'adv."pie"',
    ]

    with duckdb.connect() as conn:
        register_arrow_table(conn, table_name="agg_team_season", table=agg_team_table)
        register_arrow_table(conn, table_name="dim_team", table=dim_team_table)
        register_arrow_table(conn, table_name="fct_team_game", table=team_fact_table)
        register_arrow_table(conn, table_name="boxscore_team_game", table=silver_team_table)

        conn.execute(team_advanced_view.build_view_sql(team_advanced_view.VIEW_NAME, include_percentiles=False))

        return conn.execute(
            f"""
            SELECT
                ats."team_id",
                ats."current_team_sk",
                ats."season_year",
                ats."season_start_year",
                ats."raw_season_type_code",
                ats."season_type",
                ats."games_played",
                {", ".join(team_metric_selects)}
            FROM "agg_team_season" AS ats
            LEFT JOIN "{team_advanced_view.VIEW_NAME}" AS adv
                ON ats."team_id" = adv."team_id"
               AND ats."season_year" = adv."season_year"
               AND ats."season_type" = adv."season_type"
            """
        ).fetch_arrow_table()


def build_team_percentile_rows(
    agg_team_table: pa.Table,
    advanced_source_table: pa.Table,
) -> list[dict[str, Any]]:
    advanced_rows_by_key: dict[tuple[int, str, str], dict[str, Any]] = {}
    for row in advanced_source_table.to_pylist():
        team_id = to_int_or_none(row.get("team_id"))
        season_year = to_str_or_none(row.get("season_year"))
        season_type = to_str_or_none(row.get("season_type"))
        if team_id is None or season_year is None or season_type is None:
            continue
        advanced_rows_by_key[(team_id, season_year, season_type)] = row

    rows: list[dict[str, Any]] = []
    for row in agg_team_table.to_pylist():
        team_id = to_int_or_none(row.get("team_id"))
        season_year = to_str_or_none(row.get("season_year"))
        season_type = to_str_or_none(row.get("season_type"))
        raw_season_type_code = to_str_or_none(row.get("raw_season_type_code"))
        if team_id is None or season_year is None or season_type is None or raw_season_type_code is None:
            continue

        enriched = dict(row)
        enriched.update(advanced_rows_by_key.get((team_id, season_year, season_type), {}))
        rows.append(enriched)

    attach_percentiles(
        rows,
        metric_specs=TEAM_PERCENTILE_SPECS,
        group_fields=("season_year", "raw_season_type_code"),
    )

    final_rows: list[dict[str, Any]] = []
    for row in rows:
        final_row: dict[str, Any] = {
            "team_id": to_int_or_none(row.get("team_id")),
            "current_team_sk": to_int_or_none(row.get("current_team_sk")),
            "season_year": to_str_or_none(row.get("season_year")),
            "season_start_year": to_int_or_none(row.get("season_start_year")),
            "raw_season_type_code": to_str_or_none(row.get("raw_season_type_code")),
            "season_type": to_str_or_none(row.get("season_type")),
        }
        for field_name in TEAM_PERCENTILE_FIELDS:
            final_row[field_name] = to_int_or_none(row.get(field_name))
        final_rows.append(final_row)

    final_rows.sort(
        key=lambda row: (
            row.get("team_id") if row.get("team_id") is not None else -1,
            row.get("season_start_year") if row.get("season_start_year") is not None else -1,
            row.get("raw_season_type_code") or "",
        )
    )
    return final_rows


def finalize_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    run_ts = datetime.now(timezone.utc)
    for index, row in enumerate(rows, start=1):
        row["team_season_percentiles_sk"] = index
        row["record_source"] = RECORD_SOURCE
        row["created_at_utc"] = run_ts
        row["updated_at_utc"] = run_ts
    return rows


def main() -> None:
    s3_client = boto3.client("s3")

    agg_team_table = read_parquet_table_from_s3(s3_client, TEAM_AGG_SOURCE_KEY, list(TEAM_AGG_SCHEMA.names))
    dim_team_table = read_parquet_table_from_s3(s3_client, DIM_TEAM_SOURCE_KEY, DIM_TEAM_REQUIRED_COLUMNS)
    team_fact_table = read_parquet_table_from_s3(s3_client, TEAM_FACT_SOURCE_KEY, list(TEAM_FACT_SCHEMA.names))
    silver_team_table = read_parquet_table_from_s3(s3_client, SILVER_TEAM_SOURCE_KEY, SILVER_TEAM_REQUIRED_COLUMNS)

    advanced_source_table = _build_team_advanced_source_table(
        agg_team_table,
        dim_team_table,
        team_fact_table,
        silver_team_table,
    )
    rows = build_team_percentile_rows(agg_team_table, advanced_source_table)

    write_parquet_to_s3(finalize_rows(rows), TARGET_SCHEMA, DESTINATION_KEY, s3_client)
    print(f"Wrote s3://{S3_BUCKET}/{DESTINATION_KEY}")


if __name__ == "__main__":
    main()
