...
def parse_meta_time_to_utc(value: Any) -> datetime | None:
    ...
    # Fallback for ISO-ish strings, if encountered.
...
def build_latest_game_rows(s3_client) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for obj in objects:
        ...
        try:
            ...
            row = build_game_row(payload)
        except Exception as exc:
            failed_reads += 1
            print(f"Failed to parse {key}: {type(exc).__name__}: {exc}")
            continue

        game_id = row.get("gameId") or extract_game_id_from_key(key)
        if game_id is None:
        ...
        candidate_meta_time = row.get("meta_time")
        current = latest_by_game_id.get(game_id)
        if current is None:
            latest_by_game_id[game_id] = (row, candidate_meta_time, last_modified)
            continue
    ...
    rows = [item[0] for item in latest_by_game_id.values()]
