"""
Build a canonical silver player-game defensive shot context table.
...
DESTINATION_KEY = "silver/player_game_defensive_shot_context.parquet"
...
PLAYER_REQUIRED_COLUMNS = pgpc.PLAYER_REQUIRED_COLUMNS
...
TARGET_SCHEMA = pa.schema(
    [
        ...
        pa.field("opponent_two_point_attempts_while_on_court", pa.float64()),
...
def init_player_stats(person_ids: set[int], source_method: str) -> dict[int, dict[str, Any]]:
    return {
        person_id: {
            "opponent_two_point_attempts_while_on_court": 0.0,
...
def assign_defensive_shot_attempt(
    stats: dict[int, dict[str, Any]],
    defenders: list[int],
    *,
    count_field: str,
) -> None:
    for person_id in defenders:
        player_stats = stats.get(person_id)
        if player_stats is None:
            continue
        player_stats["opponent_two_point_attempts_while_on_court"] += 1.0
        player_stats[count_field] += 1.0


def resolve_defense_players_from_lineups(
    *,
    home_team_id: int,
    away_team_id: int,
    ...
    if defense_team_id is None and offense_team_id is not None:
        if offense_team_id == home_team_id:
            defense_team_id = away_team_id
        elif offense_team_id == away_team_id:
            defense_team_id = home_team_id
    if defense_team_id == home_team_id:
        return home_players
    if defense_team_id == away_team_id:
        return away_players
    return None


def build_exact_player_game_shot_stats(
    ...
    stats = init_player_stats(set(played_players), "exact")
    ...
    for row in shot_rows:
        ...
        defenders = resolve_defense_players_from_lineups(
        ...
        if defenders is None:
            return None
        assign_defensive_shot_attempt(
            stats,
            defenders,
            count_field="exact_two_point_attempts_count",
        )

    for player_stats in stats.values():
        player_stats["exact_game_flag"] = 1
    return stats


def build_event_estimated_player_game_shot_stats(
    playbyplay_rows: list[dict[str, Any]] | None,
    event_context_rows: list[dict[str, Any]] | None,
    played_players: dict[int, dict[str, Any]],
) -> dict[int, dict[str, Any]] | None:
    ...
    for row in shot_rows:
        ...
        assign_defensive_shot_attempt(
            stats,
            defenders,
            count_field="event_estimated_two_point_attempts_count",
        )
...
def build_boxscore_estimated_player_game_shot_stats(
    played_players: dict[int, dict[str, Any]],
    ...
    *,
    ...
    for person_id, player_info in played_players.items():
        ...
        estimated_attempts = minute_share * opponent_two_pa

        player_stats = stats[person_id]
        player_stats["opponent_two_point_attempts_while_on_court"] = estimated_attempts
        player_stats["boxscore_estimated_two_point_attempts_count"] = estimated_attempts
        player_stats["boxscore_estimated_game_flag"] = 1
    return stats


def build_missing_player_game_shot_stats(played_players: dict[int, dict[str, Any]]) -> dict[int, dict[str, Any]]:
...
def choose_game_stats(
    *,
    ...
    exact_stats = build_exact_player_game_shot_stats(playbyplay_rows, on_court_rows, played_players)
    if exact_stats is not None:
        return exact_stats

    event_stats = build_event_estimated_player_game_shot_stats(
        playbyplay_rows,
        event_context_rows,
        played_players,
    )
    ...
    boxscore_stats = build_boxscore_estimated_player_game_shot_stats(
        played_players,
        team_context_map,
        game_id=game_id,
    )
    ...
    return build_missing_player_game_shot_stats(played_players)
...
def finalize_player_game_row(
    *,
    ...
    return {
        ...
        "opponent_two_point_attempts_while_on_court": pgpc.to_float_or_none(
            stats.get("opponent_two_point_attempts_while_on_court")
        )
        or 0.0,
...
def build_rows(
    *,
    ...
    played_players_by_game = pgpc.build_played_players_by_game(player_table)
    ...
    pipeline_run_id = (
        f"player_game_defensive_shot_context_{ingested_at_utc.strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
...
def main() -> None:
    ...
    write_audit_artifacts(
        ...
        bucket=S3_BUCKET,
        table_name=TABLE_NAME,
        pipeline_run_id=rows[0]["_meta_pipeline_run_id"] if rows else "player_game_defensive_shot_context_empty",
        ingested_at_utc=ingested_at_utc,
        audit_row=audit_row,
    )
