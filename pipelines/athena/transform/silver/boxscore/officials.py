...
from boxscore.sources import build_latest_cdn_boxscore_candidates, null_if_empty, to_int_or_none
...
def build_latest_official_rows(s3_client) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for game_id in sorted(latest_by_game_id):
        ...
        game = candidate.payload.get("game") or {}
        rows.extend(
            to_official_rows(
                game_id,
                game.get("officials") or [],
                source_key=null_if_empty(candidate.source_key),
                source_last_modified_utc=candidate.source_last_modified_utc,
            )
        )
...
def validate_and_dedupe_rows(
    ...
    for row in rows:
        ...
        if assignment is not None and assignment not in OFFICIAL_ASSIGNMENTS:
            warning_count += 1
