"""
Transform raw Kaggle Players CSV to a silver parquet file.

Reads:
  s3://nba-analytics-lakehouse-dev/raw/kaggle/Players.csv
...
SOURCE_KEY = "raw/kaggle/Players.csv"
DESTINATION_KEY = "silver/players.parquet"
TABLE_NAME = "players"
META_SOURCE_SYSTEM = "kaggle_players_csv"
...
def read_source_rows(s3_client) -> tuple[list[str], list[dict[str, Any]], datetime | None]:
    ...
    if not columns:
        raise ValueError("Players.csv has no header columns.")
...
def main() -> None:
    """Run transform from raw Players CSV to silver parquet."""
    s3_client = boto3.client("s3")
    ingested_at_utc = datetime.now(timezone.utc)
    pipeline_run_id = f"players_{ingested_at_utc.strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
