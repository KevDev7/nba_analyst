...
def select_incremental_game_ids_from_legacy_state(
    ...
    state_watermark = pbp_silver.parse_state_datetime(state.get("max_source_last_modified_utc"))
    ...
    for game_id, artifacts in source_index.items():
        artifact_last_modified = max(
            (
                pbp_silver.parse_state_datetime(artifact.get("last_modified_utc"))
    ...
    selection_meta = {
        ...
        "watermark_utc": pbp_silver.state_datetime_to_iso(state_watermark),
        "effective_watermark_utc": pbp_silver.state_datetime_to_iso(effective_watermark),
        "lookback_minutes": INCREMENTAL_LOOKBACK_MINUTES,
    }
